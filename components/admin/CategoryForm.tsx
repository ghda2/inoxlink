...
export function CategoryForm({ initialData, onSubmit, isEdit = false }: any) {
            ...
            <form onSubmit={form.handleSubmit(async (v) => { setIsPending(true); await onSubmit(v); })} className="space-y-6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Link href="/admin/categories"><Button variant="outline" size="icon" type="button"><ArrowLeft className="h-4 w-4" /></Button></Link>
                        <h1 className="text-3xl font-bold">{isEdit ? "Editar Categoria" : "Nova Categoria"}</h1>
                    </div>
